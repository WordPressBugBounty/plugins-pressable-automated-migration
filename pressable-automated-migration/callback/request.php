...
if (!class_exists('PBLCallbackRequest')) :
	class PBLCallbackRequest {
		public $params;
		public $method;
		public $wing;
		public $is_afterload;
			...
			if (array_key_exists('bvprms', $in_params) && isset($in_params['bvprms'])) {
				...
				if ($this->compare_mac($this->bvprmsmac, $calculated_mac) === true) {
					...
					if (array_key_exists('sersafe', $in_params)) {
						...
						$in_params[$key] = PBLCallbackRequest::serialization_safe_decode($in_params[$key]);
					}
		...
		public static function serialization_safe_decode($data) {
			if (is_array($data)) {
				$data = array_map(array('PBLCallbackRequest', 'serialization_safe_decode'), $data);
		...
		public function verify($data, $sig, $sighshalgo) {
			...
			$public_key_str = PBLWPFileSystem::getInstance()->getContents($key_file);
