...
if (!class_exists('PBLRespStream')) :

	class PBLStream extends PBLCallbackBase {
		public $bvb64stream;
		public $bvb64cksize;
		public $checksum;
		...
		public static function startStream($account, $request) {
			...
			$stream = new PBLRespStream($request);
			if ($request->isAPICall()) {
				$stream = new PBLHttpStream($request);
				if (!$stream->connect()) {
					$apicallstatus = array(
...
class PBLRespStream extends PBLStream {
	public $bvboundry;
...
class PBLHttpStream extends PBLStream {
	var $user_agent = 'PBLHttpStream';
	var $host;
	var $port;
	var $timeout = 20;
	var $conn;
