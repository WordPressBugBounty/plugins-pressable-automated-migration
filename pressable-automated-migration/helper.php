...
if (!class_exists('PBLHelper')) :
	class PBLHelper {
		...
		public static function fileRemovePattern($fname, $pattern, $is_regex = false) {
			...
			if (!PBLWPFileSystem::getInstance()->exists($fname)) {
				return;
			}

			$content = PBLWPFileSystem::getInstance()->getContents($fname);
			if ($content !== false) {
				if ($is_regex !== false) {
				...
				if ($content !== $modified_content) {
					PBLWPFileSystem::getInstance()->putContents($fname, $modified_content,
							PBLWPFileSystem::getInstance()->getchmodOctal($fname));
				}
			}
		}
